...
describe("ma505mAlignmentFilter", () => {
  it("aligns UP when line >= MA", () => {
    const snap = buildSnapshot({
      currentSide: "up",
      line: 105,
      context: { ...emptyContext(), ma50x5m: 100 },
    });
    expect(ma505mAlignmentFilter.classify(snap, snap.context)).toBe(true);
  });

  it("DOWN against a bullish MA regime is not aligned", () => {
    const snap = buildSnapshot({
      currentSide: "down",
      line: 105,
      context: { ...emptyContext(), ma50x5m: 100 },
    });
    expect(ma505mAlignmentFilter.classify(snap, snap.context)).toBe(false);
  });
  ...
  it("skips when MA unavailable", () => {
    ...
    expect(ma505mAlignmentFilter.classify(snap, snap.context)).toBe("skip");
  });
});
