...
describe("ema205mAlignmentFilter", () => {
  it("aligns UP when line >= EMA", () => {
    const snap = buildSnapshot({
      currentSide: "up",
      line: 105,
      context: { ...emptyContext(), ema20x5m: 100 },
    });
    expect(ema205mAlignmentFilter.classify(snap, snap.context)).toBe(true);
  });
  ...
  it("DOWN against a bullish EMA regime is not aligned", () => {
    ...
    expect(ema205mAlignmentFilter.classify(snap, snap.context)).toBe(false);
  });
  ...
  it("skips when EMA unavailable", () => {
    ...
    expect(ema205mAlignmentFilter.classify(snap, snap.context)).toBe("skip");
  });
});
